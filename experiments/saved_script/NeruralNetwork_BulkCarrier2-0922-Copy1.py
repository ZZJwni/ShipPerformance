...
result.to_csv(output_figpath / 'pred_bulk_label2.csv')


# In[3]:


result = pd.read_csv(output_figpath / 'pred_bulk_label2.csv', index_col=0, parse_dates=['utc'])
...
for idx in tqdm_notebook(test_error_by_record_.index):
    plot_tw_speed(df=result.query('record_index == @idx'), colors=colors,
                  labels=labels, output_figpath='fig/tw_speed_bulk2_0922/')
    plot_og_speed(df=result.query('record_index == @idx'), colors=colors,
                  labels=labels_2, output_figpath='fig/tw_speed_bulk2_0922/')
