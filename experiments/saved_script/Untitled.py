...
for label in [0,1,2,3,4]:
    results.append(pd.read_csv(output_datapath / f'feats_2_{label}_speed_1.csv', index_col=0, parse_dates=['utc']))
df = (pd
...
for label in [0, 1, 2, 3, 4]:
    ...
    evaluate_model(y_test=test_pred['og_speed'].values, y_test_pred=test_pred['pred'].values,
                   y_train=train_val_pred['og_speed'].values, y_train_pred=train_val_pred['pred'].values)
...
labels = ['og_speed', 'og_speed_train_pred', 'og_speed_test_pred']
...
testmae_by_record = df_.groupby('record_index').apply(lambda col : mean_absolute_error(col['og_speed'], col['pred']))
...
for idx in tqdm_notebook(testmae_by_record.sort_values().index):
    plot_pred(df=df_.query('record_index == @idx'), colors=colors,
              labels=labels, output_figpath='fig/fig_2_speed_2/')
