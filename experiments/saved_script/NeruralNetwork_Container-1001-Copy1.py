...
model._model.get_weights()[0].shape
...
result.to_csv(output_figpath / 'pred_container_1.csv')


# In[7]:


result = pd.read_csv(output_figpath / 'pred_container_1.csv', index_col=0, parse_dates=['utc'])
df_ships = pd.read_csv(feature_path / ship_filename, 
                       index_col=0)
...
'querycolors = sns.color_palette(palette='muted', n_colors=4)
...
for idx in tqdm_notebook(test_error_by_record_.index):
    plot_tw_speed(df=result.query('record_index == @idx'), colors=colors,
                  labels=labels, output_figpath='fig/tw_speed_container_1001_2/')
