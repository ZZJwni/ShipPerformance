...
df = pd.merge(left=df_feats, right=df_ships[ship_features + ['ship_id', 'label']], on='ship_id', how='inner')


# In[14]:


df.groupby('label').apply(lambda x: len(x['record_index'].unique()))
...
result.to_csv(output_figpath / 'pred_container_label0.csv')


# In[3]:


result = pd.read_csv(output_figpath / 'pred_bulk_label3.csv', index_col=0, parse_dates=['utc'])
...
for idx in tqdm_notebook(test_error_by_record_.index):
    plot_tw_speed(df=result.query('record_index == @idx'), colors=colors,
                  labels=labels, output_figpath='fig/tw_speed_container0_0924/')
