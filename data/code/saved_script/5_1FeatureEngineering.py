...
df_.loc[df_['foc_me'] == 54.6, 'rpm'].tail(50)
...
# df_mod = df_mod.assign(crnt_effect=df_mod['crnt_u'] * np.cos(
#     df_mod['course']) + df_mod['crnt_v'] * np.sin(df_mod['course']),
#                        wind_effect=df_mod['wind_u'] * np.cos(
#     df_mod['course']) + df_mod['wind_v'] * np.sin(df_mod['course'])
# df_mod = df_mod.assign(trim=df_mod['draft_fore']-df_mod['draft_aft'],
#                        draft_mean=(df_mod['draft_fore']+df_mod['draft_aft'])/2)
...
df_mod.loc[:, cols + ['utc']].to_csv(data_path / 'processed/leg_top5_feat.csv')
