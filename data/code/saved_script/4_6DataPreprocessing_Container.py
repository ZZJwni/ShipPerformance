...
db_container = pd.read_csv(data_path / 'processed/db_container_ship.csv')
...
df_ = df.loc[df['ship_id'].isin(db_container['ship_id']), :].copy()
del df
df = df_
...
df.reset_index(drop=True, inplace=True)
df_ = pd.merge(left=df, right=db_container[[
...
# 6. Remove outliers of og_speed, rpm and foc_me
df.reset_index(drop=True, inplace=True)
...
df.loc[:, 'norm_foc_me'] = df['foc_me'] / (df['power_at_mcr'] * 0.745699872)
df.loc[:, 'norm_rpm**3'] = df['norm_rpm'] ** 3

course = df['course'] / 360 * 2 * np.pi
crnt_lg =  df['crnt_u'].values * np.sin(course) + df['crnt_v'].values * np.cos(course)
tw_speed = df['og_speed'] - crnt_lg * 3600 / 1852
df.loc[:, 'norm_tw_speed'] = tw_speed / df['speed_at_mcr']
...
#     print(len(df.loc[idx, ['norm_rpm', 'norm_foc']].values))
    scores, labels =         KNN_outlier(df.loc[idx, ['norm_rpm**3', 'norm_foc_me', 'norm_tw_speed']].values)
...
df = (df
      ...
      .drop(['norm_rpm', 'norm_foc_me', 'norm_rpm**3', 
             'outlier_score', 'outlier_label', 'norm_tw_speed',
             'draft', 'speed_at_mcr', 'rpm_at_mcr', 'power_at_mcr'
             ], axis=1))
print('The number of samples after : ', len(df))
...
#7. Remove ship_id with too little records again
...
df.to_csv(data_path / 'processed/df_container_remove_outliers.csv')


# ###### Container Ship continous records
...
df = pd.read_csv(data_path / 'processed/df_container_remove_outliers.csv',
...
# sns.distplot(a=df_utc['s1'][lambda s: s<24], kde=False, label='s1', ax=ax_1)
# sns.distplot(a=df_utc['s2'][lambda s: s<24], kde=False, label='s2', ax=ax_2)
...
internal = 6
df_utc.loc[:, 'isolated'] = (df_utc['s1'] > internal) & (df_utc['s2'] > internal)
df_utc.loc[:, 'start'] = (df_utc['s1'] > internal) & (df_utc['s2'] <= internal)
df_utc.loc[:, 'end'] = (df_utc['s1'] <= internal) & (df_utc['s2'] > internal)
df_utc.loc[:, 'inner'] = (df_utc['s1'] <= internal) & (df_utc['s2'] <= internal)

print('The ratio of isolated : ', len(df_utc.query('isolated == True')) / len(df_utc))
print('The ratio of start : ', len(df_utc.query('start == True')) / len(df_utc))
print('The ratio of end : ', len(df_utc.query('end == True')) / len(df_utc))
print('The ratio of inner : ', len(df_utc.query('inner == True')) / len(df_utc))

print('The Number of kept records : ', len(df_utc.query('isolated == False')))
print('The Number of total records : ', len(df_utc))
...
print('The ratio of kept records : ', len(df_) / len(df))
del df
df = df_
...
# In each continous record, we remove records which foc_me is too small
...
# og_speed_too_small = df.groupby('record_index')['og_speed'].apply(
#     lambda s: s < .2 * np.max(s))

# df.loc[:, 'og_speed_too_small'] = og_speed_too_small
# print('og_speed_too_small:\n', df['og_speed_too_small'].value_counts())
# df_ = df.query('og_speed_too_small == True')
# dropped_index = df_['record_index'].unique()
# print(len(df.loc[df['record_index'].isin(dropped_index), :]) / len(df))
...
# In each continous record, we remove records which
# 1. foc_me vary too little
# 2. foc_me vary too much
diff = df.groupby('record_index')['foc_me'].apply(
    lambda s: (s.diff(-1) /s) * 100 )
df.loc[:, 'foc_diff'] = np.abs(diff.fillna(0))
    

max_diff = df.groupby('record_index').apply(
    lambda col: np.max(col['foc_diff']))
print('The ratio of record_index that will be removed : ', len(
    (max_diff[lambda s: (s < 1) | (s > 100)])) / len(max_diff))
dropped_records = max_diff[lambda s: (s < 1) | (s > 100)].index

print('The number of records before : ', len(df))
df = df.loc[~df['record_index'].isin(dropped_records), :]
print('The number of records after : ', len(df))
...
columns = ['ship_id', 'client', 'course', 'crnt_u', 'crnt_v', 'draft_aft',
           'draft_fore', 'foc_me', 'og_speed', 'rpm', 'sea_dir', 'sea_ht',
           'sea_per', 'sig_ht', 'swl_dir', 'swl_ht', 'swl_per', 'utc', 'wind_u',
           'wind_v','record_length', 'record_index']


# In[37]:


df[columns].to_csv(data_path / 'processed/record_container.csv')
