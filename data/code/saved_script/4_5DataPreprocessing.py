...
# ship_db = ship_db.loc[ship_db['wni_ship_num'].isin(ship_ids), :]
...
# ship_db = ship_db_2.merge(right=ship_db_1[['wni_ship_num', 'date_built_year', 'disp', 'power_at_mcr']], on='wni_ship_num', how='left', copy=False)
# ship_db = ship_db.loc[~np.isnan(ship_db['wni_ship_num']), :]
# ship_db.loc[:, 'wni_ship_num'] = ship_db['wni_ship_num'].astype(np.int64)
...
df = pd.read_csv(filepath_or_buffer=data_path / 'processed/new_1/df_without_na.csv', 
                     index_col=0, 
                     parse_dates=['utc'], 
                     dtype=dict.fromkeys(['course', 'crnt_u', 'crnt_v',
                                          'draft_aft', 'draft_fore', 'foc_me',
...
X.apply(lambda col: np.sum(np.isnan(col))) / len(ship_db)
...
n_clusters = 4
clustering = KMeans(n_clusters=n_clusters, n_init=30)


# In[115]:


clustering.fit(X.drop(['date_built_year', 'power_at_mcr', 'speed_at_mcr', 'rpm_at_mcr'], axis=1))
...
from itertools import combinations
cols = ['length', 'breadth', 'depth', 'draft', 'gt', 'dwt']
iters = list(combinations(cols, 2))
fig, axes = plt.subplots(len(iters),1,  figsize=(8, len(iters) * 8))
...
data.to_csv(data_path / 'processed/record_bulk_new.csv')
